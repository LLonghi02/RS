...
import numpy as np
import pandas as pd
# Ensure that long strings are not truncated when saving latex or csv
...
class ResultFolderLoader(object):
    ...
    def generate_latex_results(self, output_file_path,
        ...
        if collapse_multicolumn_if_needed and (len(metrics_list) == 1):
            result_dataframe.columns = ['@'.join([self._get_metric_label(col[1]), str(col[0])]).strip() for col in result_dataframe.columns.values]
        ...
        if highlight_best:
            ...
            result_dataframe = result_dataframe.applymap(lambda x:"{:.{n_decimals}f}".format(x, n_decimals = n_decimals))
